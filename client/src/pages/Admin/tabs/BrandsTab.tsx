      ...
      key: 'status', header: '상태', render: (b) => {
        ...
        return (
          <div style={{ display: 'flex', gap: '4px', alignItems: 'center', flexWrap: 'wrap' }}>
            <Badge color={b.isActive ? 'green' : 'elephant'} variant="weak" size="sm">{b.isActive ? ACTIVE_STATUS_LABELS.brand.active : ACTIVE_STATUS_LABELS.brand.inactive}</Badge>
            {b.isActive && count === 0 && <Badge color="yellow" variant="weak" size="xs">상품 0종</Badge>}
          </div>
        );
      }
    },
...
const BrandFormModal: React.FC<{ isOpen: boolean; onClose: () => void; brand: AdminBrand | null; onSuccess: () => void }> = ({ isOpen, onClose, brand, onSuccess }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={brand ? '브랜드 수정' : '새 브랜드'} footer={
      ...
      <form onSubmit={handleSubmit} className="admin-form-body">
        <div>
          ...
          <TextField variant="box" value={formData.code} onChange={e => setFormData({ ...formData, code: e.target.value })} required disabled={!!brand} />
        </div>
        <div>
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: SPACING[2], alignItems: 'center' }}>
            <TextField
              ...
              onChange={e => { setFormData({ ...formData, imageUrl: e.target.value }); setImageError(false); }}
              placeholder="https://..."
              style={{ flex: 1 }}
            />
          ...
          {formData.imageUrl && imageError && (
            <div style={{ padding: '8px 12px', borderRadius: '8px', background: 'var(--color-red-50)', textAlign: 'center', marginTop: SPACING[2] }}>
              <span style={{ fontSize: '12px', color: 'var(--color-error)' }}>이미지를 불러올 수 없습니다. URL을 확인해주세요.</span>
            </div>
          )}
        </div>
      </form>
