...
const InquiriesTab = () => {
  ...
  return (
    <div className="admin-tab">
      ...
      {/* Detail / Answer Modal */}
      <Modal isOpen={detailOpen} onClose={() => setDetailOpen(false)} title="문의 상세" size="lg" footer={selectedInquiry ? (
        <div className="flex gap-2 w-full">
          <Button variant="ghost" onClick={() => setDetailOpen(false)} type="button" fullWidth>닫기</Button>
          {selectedInquiry.status !== 'CLOSED' && (
            <>
              <Button variant="ghost" onClick={handleClose} type="button" style={{ color: COLORS.grey600 }} fullWidth>종료 처리</Button>
              <Button variant="primary" onClick={handleAnswer} type="button" loading={answerLoading} disabled={!answer.trim()} fullWidth>답변 저장</Button>
            </>
          )}
        </div>
      ) : undefined}>
          ...
          <div style={{ padding: SPACING[4] }}>
            ...
            <div>
              ...
              <textarea
                ...
                rows={6}
                placeholder={selectedInquiry.status === 'CLOSED' ? '종료된 문의입니다' : '답변을 입력하세요...'}
                maxLength={5000}
