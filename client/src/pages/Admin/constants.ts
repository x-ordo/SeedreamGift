...
import {
  ...
  MessageSquare, Settings, BookOpen, RefreshCw, Bell, Building2,
} from 'lucide-react';
import {
  BRAND_OPTIONS,
  ORDER_STATUS_MAP,
...
export type AdminTab =
  | 'dashboard' | 'users' | 'sessions' | 'partners' | 'products' | 'brands' | 'vouchers'
  | 'orders' | 'tradeins' | 'gifts' | 'refunds' | 'cash-receipts' | 'notices' | 'events'
  | 'faqs' | 'inquiries' | 'notification-channels' | 'configs' | 'audit-logs';
...
export const CASH_RECEIPT_STATUS_OPTIONS = [
  { value: 'PENDING', label: '처리중', color: 'yellow' },
  { value: 'ISSUED', label: '발급완료', color: 'green' },
  { value: 'FAILED', label: '발급실패', color: 'red' },
  { value: 'CANCELLED', label: '취소됨', color: 'elephant' },
];

export const CASH_RECEIPT_STATUS_COLOR_MAP = new Map(CASH_RECEIPT_STATUS_OPTIONS.map(o => [o.value, o.color]));
...
export const ADMIN_TABS: AdminTabConfig[] = [
  ...
  { id: 'partners', label: '파트너 사업자', icon: Building2, group: 'members', title: '파트너 사업자 정보', description: '파트너사의 사업자 등록 정보를 검토하고 인증합니다.' },
  ...
  // { id: 'cash-receipts', label: '현금영수증', icon: Receipt, group: 'transactions', title: '현금영수증 관리', description: '현금영수증 발급 내역을 조회하고 관리합니다.' },
  ...
  { id: 'notification-channels', label: '알림 채널 관리', icon: Bell, group: 'system', title: '알림 채널 관리', description: '이메일, 카카오, 텔레그램, 팝빌 채널을 관리합니다.' },
...
export const FULFILLMENT_TYPE_OPTIONS = [
  { value: 'STOCK', label: '수동 PIN 업로드' },
  { value: 'API', label: '자동 발급' },
] as const;
