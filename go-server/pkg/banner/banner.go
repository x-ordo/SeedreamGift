...
func Print(version, buildTime string) {
	logo := `
  ____  _____ _____ ____  ____  _____    _    __  __    ____ ___ _____ _____
 / ___|| ____| ____|  _ \|  _ \| ____|  / \  |  \/  |  / ___|_ _|  ___|_   _|
 \___ \|  _| |  _| | | | | |_) |  _|   / _ \ | |\/| | | |  _ | || |_    | |
  ___) | |___| |___| |_| |  _ <| |___ / ___ \| |  | | | |_| || ||  _|   | |
 |____/|_____|_____|____/|_| \_\_____/_/   \_\_|  |_|  \____|___|_|     |_|
                                                                             `

	fmt.Println(colorCyan + colorBold + logo + colorReset)
	fmt.Printf("%s%s SEEDREAM GIFT API Server %s%s\n", colorBlue, colorBold, colorReset, colorCyan+VersionInfo(version, buildTime)+colorReset)
