...
const NoticesTab = () => {
  ...
  const columns: Column<AdminNotice>[] = [
    ...
    { key: 'status', header: '상태', render: (n) => <Badge color={n.isActive ? 'green' : 'elephant'} variant="weak" size="small">{n.isActive ? '게시중' : '비공개'}</Badge> },
    {
      key: 'date', header: '작성일',
      render: (n) => (
        <div>
    ...
    },
    {
      key: 'actions', header: '작업', align: 'right', render: (n) => (
        <div className="admin-actions">
          ...
          <Button variant="ghost" size="sm" style={{ color: COLORS.error }} onClick={() => deleteConfirm.openConfirm(n.id, n.title)}>삭제</Button>
        </div>
      )
    }
  ];
...
const NoticeFormModal: React.FC<{ isOpen: boolean; onClose: () => void; notice: AdminNotice | null; onSuccess: () => void }> = ({ isOpen, onClose, notice, onSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      if (notice) await adminApi.updateNotice(notice.id, formData);
      else await adminApi.createNotice(formData);
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={notice ? '공지사항 수정' : '새 공지사항'}>
      <form onSubmit={handleSubmit} className="admin-form-body">
        ...
        {notice && notice.id > 0 ? (
          <div style={{ marginTop: SPACING[3] }}>
            <AttachmentManager targetType="NOTICE" targetId={notice.id} />
          </div>
        ) : (
          <div style={{ marginTop: SPACING[2], fontSize: '12px', color: COLORS.grey500 }}>
            첨부 파일은 공지사항 저장 후 추가할 수 있습니다
          </div>
        )}
