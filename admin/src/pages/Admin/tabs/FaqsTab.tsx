...
const FaqsTab = () => {
  ...
  const columns: Column<AdminFaq>[] = [
    ...
    { key: 'category', header: '카테고리', render: (f) => <Badge color="blue" variant="weak" size="small">{f.category}</Badge> },
    ...
    { key: 'status', header: '상태', render: (f) => <Badge color={f.isActive ? 'green' : 'elephant'} variant="weak" size="small">{f.isActive ? '공개' : '비공개'}</Badge> },
    {
...
const FaqFormModal: React.FC<{ isOpen: boolean; onClose: () => void; faq: AdminFaq | null; onSuccess: () => void }> = ({ isOpen, onClose, faq, onSuccess }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={faq ? 'FAQ 수정' : '새 FAQ'}>
