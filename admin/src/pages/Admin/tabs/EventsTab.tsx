...
const EventsTab = () => {
  ...
  const columns: Column<AdminEvent>[] = [
    ...
    { key: 'status', header: '상태', render: (e) => <Badge color={e.isActive ? 'green' : 'elephant'} variant="weak" size="small">{e.isActive ? ACTIVE_STATUS_LABELS.event.active : ACTIVE_STATUS_LABELS.event.inactive}</Badge> },
    {
...
const EventFormModal: React.FC<{ isOpen: boolean; onClose: () => void; event: AdminEvent | null; onSuccess: () => void }> = ({ isOpen, onClose, event, onSuccess }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={event ? '이벤트 수정' : '새 이벤트'}>
      <form onSubmit={handleSubmit} className="admin-form-body">
        ...
        {event && event.id > 0 ? (
          <div style={{ marginTop: SPACING[3] }}>
            <AttachmentManager targetType="EVENT" targetId={event.id} />
          </div>
        ) : (
          <div style={{ marginTop: SPACING[2], fontSize: '12px', color: COLORS.grey500 }}>
            첨부 파일은 이벤트 저장 후 추가할 수 있습니다
          </div>
        )}
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="이벤트 상세">
      <div style={{ padding: SPACING[4] }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: SPACING[2], marginBottom: SPACING[3] }}>
          ...
          <Badge color={status.color} variant="weak" size="small">{status.label}</Badge>
          {event.isFeatured && <Badge color="yellow" variant="weak" size="small">메인노출</Badge>}
        </div>
        ...
        <div style={{ display: 'flex', gap: SPACING[4], fontSize: '13px', color: COLORS.grey400, marginBottom: SPACING[4], marginTop: SPACING[4] }}>
          <span>조회수: {event.viewCount || 0}</span>
          <span>생성: {new Date(event.createdAt).toLocaleDateString('ko-KR')}</span>
        </div>
        <div className="admin-actions">
          <Button variant="ghost" onClick={onClose}>닫기</Button>
          <Button variant="primary" onClick={onEdit}>수정하기</Button>
        </div>
      </div>
    </Modal>
  );
